...
int main(void) {
	int N = 32 * 1024 * 256;
	double threadsPerBlock = 16;
	while (threadsPerBlock < pow(2.0, 15)) {
		...
		if (threadsPerBlock == 16) {
			...
			printf("%-100s%-15s%-15s%-15s\n", "Block_size", " CPUtoGPU(ms)",
					" Kernel(ms)", " GPUtoCPU(ms)");
		}
		printf("%-102.0f%-15f%-15f%-15f\n", (threadsPerBlock), elapsedTime,
